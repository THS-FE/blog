...
console.log(sallaryLevel1(200));  // 1120
console.log(sallaryLevel2(220));  // 40000
```

chorme开发者工具 > Sources > Call Stack(函数调用栈) / Scope(作用域) > Global(var 全局) / Local(局部) / Closure(闭包) / Script(let 作用域)
...
const f = _.flowRight(join('-'), log, split(' '), _.toLower);
