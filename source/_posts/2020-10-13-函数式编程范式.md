...
> 注：本文部分代码涉及到一些JavaScript新特性，需要提前了解一下：[JavaScript 新特性](/2020/08/24/JavaScript新特性/) 
...
> 声明式编程：以数据结构的形式来表达程序执行的逻辑。它的主要思想是告诉计算机应该做什么，但不指定具体要怎么做。SQL 语句就是最明显的一种声明式编程的例子，我们只需要定义好该如何处理数据，程不需要指定具体实现，就可以查询到我们需要的数据。
...
7. 引用透明：函数的运行不依赖于外部变量或"状态"，只依赖于输入的参数，任何时候只要参数相同，引用函数所得到的返回值总是相同的。
...
函数式编程范式只是一种对程序编程思维的一种概论，而具体的实现则通过柯里化（第5章）、函数组合（第5章）、函子来实现。
...
// 属性赋值示例
// const objController = {
//   getKeys(obj) { // ES6属性简写，等同于getKeys: getKeys(obj)
...
const pay = once((money) => {  // 传入一个函数，通过输出模拟支付过程和结果
...
- _.curry(fn)
	...
	- 功能：创建一个函数，该函数接受fn的参数。如果fn所需的参数都被提供则执行fn并返回结果，否则**继续返回该函数并等待接收剩余的参数**。
