...
同样是添加资源，使用Put只能添加一条数据，无论执行多少次。而使用Post可能会执行一次就添加一条新数据。

补充：

1. 所有通用的服务器必须支持GET和HEAD方法，其他方法是可选的。

这和大部分的实现还算是比较吻合，因为一般的服务器是做网站用途的，在地址栏中访问对应的url地址、<img>引入图片、JS文件引入、CSS文件引入等都是发送的GET请求。

2. 无论是GET还是POST都没有限制请求参数的长度和位置（是在URL后还是在消息体message body里）。
...
3. 由于以上2的缘故，再加上有些服务器会记录和打印GET请求的地址，因此，POST比GET相对安全一些。但是使用抓包工具的话，只要不加密，参数都能轻而易举地获取。

4. 华为手机浏览器不支持Put方法，因此有些公司会规定添加和修改都使用Post。

5. GET和POST请求参数的大小主要是由浏览器和Web服务器决定的。以下是利用node.js做服务端进行的一系列测试（感谢 [橘子君](https://github.com/SYJjuzijun)）：
...
也就是说除了**在地址栏中输入URL发送Get请求有明显的限制，直接通过代码发送请求，Get(80K左右)和Post(100K)发送的数据大小相差不多**。

### 误区

1. 一般认为HTTP请求方法与CRUD操作的对应关系如下：
