...
- 引用类型：**对象类型Object type**，Object、Array、Date、Function、RegExp等
...
综上，**JSON.parse()和JSON.stringify()是完全的深拷贝**。但是，**这种方法虽然可以实现数组或对象深拷贝,但不能处理函数和正则**，因为这两者基于JSON.stringify和JSON.parse处理后，得到的正则就不再是正则（变为空对象），得到的函数就不再是函数（变为null）了，举个例子如下：
