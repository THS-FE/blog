...
默认会通过以前选择过的包管理工具 yarn 或 NPM 来安装依赖。如果想修改的话，在windows命令行中运行:
...
1. 在根目录下添加 vue.config.js 文件来扩展webpack配置
...
2. 在src文件夹下创建layouts、plugins、services、utils文件夹
...
4. 在assets文件夹下创建fonts、icons、images、styles文件夹，删除 logo.png
5. 删除 components 文件夹下的 HelllWorld.vue，views 文件夹下的 About.vue, Home.vue
6. 按kebab-case命名法，修改所有不符合规范的文件和文件夹和对应文件中的引用(App.vue -> app.vue, Home.vue -> home.vue)
  ...
  import Home from '../views/home.vue';  // 之前是Home.vue
