...
cover: 2020/09/14/Vue3-js模板项目创建/template.jpg
categories:
  - [JS框架, Vue.js3]
tags:
  - Vue.js3
...
创建 hooks、layouts、plugins、services、utils 文件夹（每个文件夹的作用在下方改造后的目录结构中有说明）

#### 在 assets 文件夹下

创建 fonts、icons、images、styles 文件夹，删除 logo.png （每个文件夹的作用在下方改造后的目录结构中有说明）
...
#### 按 kebab-case 命名法，修改所有不符合规范的文件和对应文件中的引用

App.vue -> app.vue, Home.vue -> home.vue
...
import './plugins'; // 引入所需插件 TODO 根据具体插件添加
...
```javascript
// index.ts  删除了about 路由,自动合并当前文件夹下的文件到路由中去
...
```typescript
// login.vue 添加store actions以及 路由hook 的调用示例
...
```scss
// style.scss 全局变量都写在这里，默认引入 通用样式，被main.ts引入

@import './common.scss';
```

#### 在根目录下添加 vue.config.js 文件来扩展 webpack 配置
