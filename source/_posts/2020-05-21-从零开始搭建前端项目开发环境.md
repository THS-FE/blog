...
以上配置的意思是要兼容全球范围内使用量大于1%的浏览器和它们最新的两个版本，加上IE 9-11。可以看出这里既可以指定浏览器的版本，也可以通过查询的方式来确定要支持的浏览器。详情见[Browserslist](#Browserslist-4-11-1)这一章节。
...
就能在新生成的dist文件夹中看到编译后的代码。**注：这里的和上边所有提到的“编译”一词更准确的说法应该是“转译（Transpile）”。**
...
### [针对 TypeScript](https://github.com/typescript-eslint/typescript-eslint)
...
### [Entry](https://webpack.js.org/concepts/#entry) & [Output](https://webpack.js.org/concepts/#output)
...
TS转译成JS用的是 Babel ，并没有使用 TypeScript 编译器。
...
将小于一个限制大小的文件转换为[base64 URIs](https://tools.ietf.org/html/rfc2397)。超过限制的，会默认使用 file-loader来做处理。所以这里一定要把 file-loader 也安装上。
