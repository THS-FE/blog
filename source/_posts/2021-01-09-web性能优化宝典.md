...
在上边2、3步时，构建 DOM 的过程是：从父到子，从先到后，一个一个节点构造，并且挂载到 DOM 树上的，在这个过程中，我们依次拿到上一步构造好的元素，去检查它匹配到了哪些规则，再根据规则的优先级，做覆盖和调整。整个过程中构建 DOM 树和匹配 CSS 规则是同步进行的。
...
TCP 预连接。与DNS预取方法非常相似，预连接将解析 DNS，但也会进行 TCP 握手和 TLS 协商（可选）。
